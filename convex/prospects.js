...
export const list = query({
    args: {
        status: v.optional(v.union(v.literal("new"), v.literal("contacted"), v.literal("negotiating"), v.literal("won"), v.literal("lost"))),
    ...
    handler: async (ctx, args) => {
        ...
        if (args.status) {
            return ctx.db
                .query("prospects")
                .withIndex("by_status", (q) => q.eq("resellerId", user._id).eq("status", args.status))
                .collect();
        }
        return ctx.db
            .query("prospects")
