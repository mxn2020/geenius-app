...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (!campaign || campaign.resellerId !== user._id) return null
        return campaign
    },
})
...
export const getEmails = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (!campaign || campaign.resellerId !== user._id) return []
        return ctx.db
...
export const updateEmailStatus = mutation({
    args: {
        ...
        status: v.union(
            v.literal("sent"), v.literal("opened"),
            v.literal("replied"), v.literal("bounced"), v.literal("failed")
        ),
        resendMessageId: v.optional(v.string()),
    },
    handler: async (ctx, args) => {
        const patch: Record<string, unknown> = { status: args.status }
        if (args.resendMessageId) patch.resendMessageId = args.resendMessageId
        if (args.status === "sent") patch.sentAt = Date.now()
        if (args.status === "opened") patch.openedAt = Date.now()
        if (args.status === "replied") patch.repliedAt = Date.now()
...
/** Get queued emails for a campaign (worker) */
export const getQueuedEmails = query({
    args: { campaignId: v.id("campaigns") },
    handler: async (ctx, args) => {
        return ctx.db
            .query("campaign_emails")
...
/** Insert a queued campaign email (worker) */
export const queueEmail = mutation({
    args: {
        campaignId: v.id("campaigns"),
        prospectId: v.id("prospects"),
        status: v.union(
            v.literal("queued"), v.literal("sent"), v.literal("failed")
        ),
    },
    handler: async (ctx, args) => {
        return ctx.db.insert("campaign_emails", {
...
/** Update campaign totals (worker) — accepts totalSent and totalProspects */
export const workerUpdateCampaign = mutation({
    args: {
        id: v.id("campaigns"),
...
/** Update email status by email ID (worker) — uses 'id' arg */
export const workerUpdateEmailStatus = mutation({
    args: {
        id: v.id("campaign_emails"),
        status: v.union(
            v.literal("queued"), v.literal("sent"), v.literal("opened"),
            v.literal("replied"), v.literal("bounced"), v.literal("failed")
        ),
